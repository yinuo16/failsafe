 ...
 * transition between states when a threshold of execution results is exceeded. A <i>time based</i> circuit breaker will
 * transition between states when a threshold is exceeded within a configured time period. A minimum number of
...
public class CircuitBreaker<R> extends DelayablePolicy<CircuitBreaker<R>, R> {
   ...
   * thresholds, the number of executions may vary within the measurement period.
   * </p>
   */
   ...
   * #getFailureThresholdingPeriod() failure measurement period}.
   * </p>
   */
  ...
  /**
   * Used with count based thresholding. Returns the capacity for storing execution results when performing failure
   * thresholding in the CLOSED or HALF_OPEN states. {@code 1} by default. Only the most recent executions that fit
   * within this capacity contribute to thresholding decisions.
   *
  ...
  /**
   * Used with time based thresholding. eturns the rolling time period during which failure thresholding is performed
   * when in the CLOSED or HALF_OPEN states, else {@code null} if time based failure thresholding is not configured.
   * Only the most recent executions that occurred within this rolling time period contribute to thresholding
   * decisions.
   *
  ...
  /**
   * Used with time based thresholding. Returns the minimum number of executions that must be recorded in the CLOSED or
   * HALF_OPEN states before the breaker can be opened. Returns {@code 0} by default.
   *
   * @see #withFailureThreshold(int, int, Duration)
   * @see #withFailureRateThreshold(int, int, Duration)
   */
  ...
  /**
   * Returns percentage rate of failures, from 1 to 100, that must occur when in a CLOSED or HALF_OPEN state in order to
   * open the circuit, else {@code 0} if failure rate thresholding is not configured.
   *
   * @see #withFailureRateThreshold(int, int, Duration)
   */
   ...
   * thresholding window} when in a CLOSED or HALF_OPEN state in order to open the circuit. Returns {@code 1} by
   * default.
   *
   * @see #withFailureThreshold(int)
   * @see #withFailureThreshold(int, int)
  ...
  /**
   * Used with count based thresholding. Returns the capacity for storing execution results when performing success
   * thresholding in the HALF_OPEN state. Only the most recent executions that fit within this capacity contribute to
   ...
   * thresholding window} when in a HALF_OPEN state in order to open the circuit. Returns {@code 0} by default.
   *
   * @see #withSuccessThreshold(int)
   * @see #withSuccessThreshold(int, int)
   */
   ...
   * the last 10 executions result in a failure. The number of executions must also exceed the {@code
   * failureExecutionThreshod} before the circuit can be opened.
   * <p>
   * If a {@link #withSuccessThreshold(int) success threshold} is not configured, the {@code failureThreshold} and
   * {@code failureExecutionThreshold} will also be used when the circuit breaker is in a HALF_OPEN state to determine
   ...
   * failureMeasurementPeriod} when in a CLOSED state in order to open the circuit.
   * <p>
   * If a {@link #withSuccessThreshold(int) success threshold} is not configured, the {@code failureThreshold} will also
   * be used when the circuit breaker is in a HALF_OPEN state to determine whether to transition back to open or
   * closed.
   * </p>
   *
   * @param failureThreshold The number of failures that must occur  within the {@code failureMeasurementPeriod} in
  ...
  /**
   * Configures time based failure thresholding by setting the number of failures that must occur within the {@code
   * failureMeasurementPeriod} when in a CLOSED state in order to open the circuit. The number of executions must also
   * exceed the {@code failureExecutionThreshod} before the circuit can be opened.
   * <p>
   * If a {@link #withSuccessThreshold(int) success threshold} is not configured, the {@code failureThreshold} and
   * {@code failureExecutionThreshold} will also be used when the circuit breaker is in a HALF_OPEN state to determine
   * whether to transition back to open or closed.
   ...
   *
   * @param failureThreshold The number of failures that must occur in order to open the circuit
   * @param failureExecutionThreshold The minimum number of executions that must be recorded during failure thresholding
  ...
  public synchronized CircuitBreaker<R> withFailureThreshold(int failureThreshold, int failureExecutionThreshold,
    ...
    assertFailureMeasurementPeriod(failureThresholdingPeriod);
    this.failureThreshold = failureThreshold;
   ...
   * must occur within the rolling {@code failureMeasurementPeriod} when in a CLOSED state in order to open the circuit.
   * The number of executions must also exceed the {@code failureExecutionThreshod} before * the circuit can be opened.
   * <p>
   * If a {@link #withSuccessThreshold(int) success threshold} is not configured, the {@code failureThreshold} and
   * {@code failureExecutionThreshold} will also be used when the circuit breaker is in a HALF_OPEN state to determine
   * whether to transition back to open or closed.
  ...
  private void assertFailureMeasurementPeriod(Duration failureThresholdingPeriod) {
